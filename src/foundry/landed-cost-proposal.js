...
function propose(db, ctx, membership, input) {
  ...
  if (!receipts.length) {
    return { proposed: false, waitingFor: 'receipt', purchaseOrderId: order.id,
      reason: 'The invoice has landed costs, but no goods receipt exists yet. Foundry is waiting for physical-delivery evidence.' };
  }
  const charges = [];
  for (const charge of eligible) {
    ...
    if (!sourceLine) {
      return { proposed: false, waitingFor: 'source_bill_line', purchaseOrderId: order.id,
        reason: `Foundry recorded the invoice but could not trace ${charge.label} to one exact bill line, so it did not propose a cost allocation.` };
    }
  ...
  // A different business rule may later propose value or weight, but Foundry
  ...
  return { proposed: true, documentId: created.document.id, purchaseOrderId: order.id,
    allocationMethod: 'quantity', reason: 'The invoice charge lines, posted bill, and received quantities all agree. Foundry prepared an exact quantity-based split for approval.' };
}
