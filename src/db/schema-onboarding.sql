...
CREATE TABLE IF NOT EXISTS workspace_connectors (
  ...
  provider_type     TEXT NOT NULL DEFAULT 'reference_webhook',
  ...
  expected_interval_minutes INTEGER NOT NULL DEFAULT 360,
  paused_at         TEXT,
  setup_status      TEXT NOT NULL DEFAULT 'CONNECTED',
...
CREATE TABLE IF NOT EXISTS connector_feed_events (
  ...
  status              TEXT NOT NULL CHECK (status IN ('RECEIVED','PROCESSING','COMPLETED','NEEDS_MAPPING','FAILED','REJECTED','STALE','IGNORED')),
