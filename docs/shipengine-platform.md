# ShipEngine shipping in StockChief

StockChief uses ShipEngine as a platform, not as one shared shipping account.

Each StockChief workspace receives a separate ShipEngine seller account. The business owner uses
ShipEngine Elements once to confirm the ship-from address, activate a carrier, and add the payment
method that funds that seller's labels. StockChief stores the returned seller API credential in the
...
for a ShipEngine Partner account. It cannot provide isolated seller onboarding for StockChief customers.
...
as permission to create seller accounts. Until StockChief has an Enterprise/Partner agreement, each
business may instead connect its own self-service shipping account and API key; that keeps postage
and subscription charges with that business but does not provide StockChief's embedded one-click
seller onboarding.

## Owner flow

1. The owner selects **Set up shipping** in StockChief.
2. StockChief creates an isolated ShipEngine seller and opens ShipEngine Elements in StockChief.
3. The business address is prefilled from the first active StockChief location.
4. The owner activates a carrier and adds their payment method once.
5. StockChief verifies that the seller has an active carrier before marking shipping ready.
6. Sales Orders can compare rates and purchase labels with that workspace's seller credential.

Disconnecting removes StockChief's encrypted credential and leaves the external seller, labels, and
